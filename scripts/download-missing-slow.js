/**
 * SCRIPT LENTO PARA BAIXAR IMAGENS
 * Usa delays maiores para não sobrecarregar a API
 * 
 * INSTRUÇÕES:
 * 1. Abra https://ragnatales.com.br/db/items no navegador
 * 2. Abra o DevTools (F12) → Console
 * 3. Cole este script e pressione Enter
 * 4. AGUARDE - vai demorar mais mas deve funcionar
 */
...
(async function() {
  ...
  console.log('🖼️ Baixando', MISSING_IDS.length, 'imagens (modo lento)...');
  console.log('⏰ Delay de 500ms entre cada imagem para não sobrecarregar a API');
  ...
  for (let i = 0; i < MISSING_IDS.length; i++) {
    ...
    // Tentar até 3 vezes se der erro
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (blob.size > 100) {
            ...
            // Imagem inválida, não tentar novamente
            failed++;
            downloaded = true;
          }
        } else if (response.status === 503) {
          // Service Unavailable - esperar mais e tentar novamente
          attempts++;
          retryCount++;
          console.log(`⏳ ${id}: 503 - aguardando 2s e tentando novamente (${attempts}/3)`);
    ...
    if ((i + 1) % 25 === 0 || i === MISSING_IDS.length - 1) {
      console.log(`📊 ${i + 1}/${MISSING_IDS.length} | ✅ ${success} | ❌ ${failed} | 🔄 ${retryCount} retries`);
    }
  ...
  console.log('\n' + '='.repeat(50));
  console.log(`✅ ${success} imagens baixadas`);
  console.log(`❌ ${failed} não encontradas`);
